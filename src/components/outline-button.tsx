 ...
 * CTA principal.
 */
export function OutlineButton({ children, onPress, accessibilityLabel, icon }: OutlineButtonProps) {
  return (
    <Pressable
      onPress={onPress}
      ...
      {({ pressed }) => (
        <ThemedView type={pressed ? 'backgroundSelected' : 'background'} style={styles.button}>
          <View style={styles.content}>
            {icon && <Ionicons name={icon} size={15} color={Palette.deepBlue} />}
