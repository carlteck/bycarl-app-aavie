...
export function IconChip({ name, variant = 'turquoise', size = 34, style }: IconChipProps) {
  const theme = useTheme();

  const background =
    variant === 'primary' ? theme.primary : variant === 'coral' ? theme.coralTint : theme.turquoiseTint;
  ...
  const iconColor =
    variant === 'primary' ? Palette.white : variant === 'coral' ? theme.accent : theme.turquoiseTintText;
